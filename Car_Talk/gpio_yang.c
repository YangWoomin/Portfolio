...
#define GPIO_SIZE               (180)   // 0x7E2000B0 ? 0x7E2000000 + 4 = 176 + 4 = 180
#define TIMER_BASE				(0xF2000000 + 0x00003000)
...
#define GPIO_MINOR 		0
#define GPIO_DEVICE     "gpio_yang"              /* 디바이스 장치 파일의 이름 */

volatile unsigned *gpio;                         /* I/O 접근을 위한 volatile 변수 */

static int gpio_col_pins[8] = { 17, 18, 27, 22, 23, 24, 25, 5 };
static int gpio_row_pins[8] = { 6, 12, 13, 19, 16, 26, 20, 21 };
...
{
	int i, j, k, n;
	char oneChar, delayChar;
	int delay;
	static int temp[8];

	get_user(delayChar, (char *)buff);
	get_user(oneChar, (char *)buff + 1);

	printk("%c\n", oneChar);

	switch ((int)delayChar) {
	case 1 :
		delay = 800;
		break;
	case 2 :
	...
	for (i = 0; i < 8; i++) {
		GPIO_CLR(gpio_col_pins[i]);
		GPIO_IN(gpio_row_pins[i]);
	}
	...
	for (i = 0; i < 8; i++) {
		...
		temp[0] = ascii[(int)oneChar][i];
		for (j = 0; j < 10; j++) {
			for (k = 7; k >= 0; k--) {
				for (n = 0; n < 8; n++) {
					if (((temp[k] & (1 << n)) >> n)) {
						GPIO_SET(gpio_col_pins[n]);
					}
					else {
						GPIO_CLR(gpio_col_pins[n]);
					}
				}
				GPIO_OUT(gpio_row_pins[k]);
				udelay(delay);
				GPIO_IN(gpio_row_pins[k]);
			}
		}
	}
...
int initModule(void)
{
	...
	register_chrdev(GPIO_MAJOR, GPIO_DEVICE, &gpio_fops);
	...
	for (i = 0; i < 8; i++) {
		GPIO_OUT(gpio_row_pins[i]);
		GPIO_OUT(gpio_col_pins[i]);
	}
